...
generate_sql_transform_report() {
    ...
    # SQLTransformReport.txt 파일 존재 확인
    if [ ! -f "$APP_TOOLS_FOLDER/SQLTransformReport.txt" ]; then
        echo -e "${RED}${BOLD}오류: SQLTransformReport.txt 파일을 찾을 수 없습니다.${NC}"
        echo -e "${YELLOW}파일 경로: $APP_TOOLS_FOLDER/SQLTransformReport.txt${NC}"
        exit 1
    fi
