...
As a migration expert in both Source and Target database systems, as well as MyBatis framework:

1. Apply deep knowledge of Source and Target syntax differences for direct conversion only.
2. Utilize advanced understanding of MyBatis XML mapper file structures while preserving original structure.
3. Implement direct syntax conversion without optimization or restructuring.
  ...
  Step 3. Sequential Individual File Conversion (MAIN PROCESS)
    ...
    2. Batch-Based Sequential Processing:
        ...
        2.4 Progress Tracking Per File:
            ...
            - Record any migration-level conversions used
        ...
        2.5 PostgreSQL Conversion Rules Application:
            ...
            **DATABASE MIGRATION EXPERT MODE:**
            ...
            - Apply appropriate PostgreSQL equivalents based on migration expert knowledge
            - Use direct syntax conversion without optimization or restructuring
            - Preserve original code structure and logic flow
            - Ensure semantic equivalence and identical functionality
            - Document migration-level transformations with comments
...
- `#{param:CLOB}` → `#{param:VARCHAR}` or `#{param:TEXT}`
- `#{param:BLOB}` → `#{param:BLOB}` (keep as is)
- `#{param:NUMBER}` → `#{param:NUMERIC}`
- `#{param:VARCHAR2}` → `#{param:VARCHAR}`
- `#{param:DATE}` → `#{param:TIMESTAMP}`
- `#{param:INTEGER}` → `#{param:INTEGER}` (keep as is)
- `#{param:DECIMAL}` → `#{param:NUMERIC}`
- `#{param:CHAR}` → `#{param:VARCHAR}`

**Complex Parameter Conversions:**
- `#{param,mode=IN,jdbcType=CLOB}` → `#{param,mode=IN,jdbcType=VARCHAR}`
- `#{param,mode=OUT,jdbcType=NUMBER}` → `#{param,mode=OUT,jdbcType=NUMERIC}`
- `#{param,mode=IN,jdbcType=VARCHAR2}` → `#{param,mode=IN,jdbcType=VARCHAR}`
...
- **WRONG**: `#{reqCont:CLOB}` → `#{reqCont}` (type information lost)
- **CORRECT**: `#{reqCont:CLOB}` → `#{reqCont:VARCHAR}` (type converted)
...
When applying migration-level conversions, add comments documenting the transformation:
```xml
