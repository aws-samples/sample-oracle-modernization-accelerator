...
These MCP servers are configured in the Kiro CLI agent configuration file (sample-mcp-agent-for-kiro-cli.json), allowing Kiro to invoke their tools as needed during migration tasks. The tools are automatically discovered and made available to Kiro's reasoning engine.
