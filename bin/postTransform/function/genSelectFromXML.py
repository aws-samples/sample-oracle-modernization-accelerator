...
class FunctionValidator:
    ...
    def extract_functions_from_xml(self, xml_content):
        ...
        # 먼저 XML에서 직접 CASE 문 추출 시도
        from sql_function_parser import extract_case_statements_from_xml
        case_statements = extract_case_statements_from_xml(xml_content)
        ...
        # CASE 문 필터링: 파서 결과에서 CASE 문 제거
        non_case_functions = [f for f in functions if not f.strip().upper().startswith('CASE')]
        
        # CASE 문을 XML 직접 추출 결과로 대체
        all_functions = case_statements + non_case_functions
        
        # 함수 추출 직후 MyBatis 바인딩 변수 처리 (개선된 버전)
        processed_functions = []
        for i, func in enumerate(all_functions):
            if self.debug and i == 2:  # 세 번째 함수(CASE 문)만 디버그
                print(f"🔍 MyBatis 처리 전: {func[:200]}...")
            ...
            if self.debug and i == 2:  # 세 번째 함수(CASE 문)만 디버그
                print(f"🔍 MyBatis 처리 후: {processed_func[:200]}...")
        ...
        for i, func in enumerate(functions):
            if self.debug and i == 2:  # 세 번째 함수(CASE 문)만 디버그
                print(f"🔍 후처리 전: {func[:200]}...")
            
            cleaned_func = self.post_process_extracted_function(func)
            
            if self.debug and i == 2:  # 세 번째 함수(CASE 문)만 디버그
                print(f"🔍 후처리 후: {cleaned_func[:200]}...")
            
            if cleaned_func and cleaned_func.strip():
                cleaned_functions.append(cleaned_func)
        
        if self.debug and cleaned_functions:
            print(f"🔍 extract_functions_from_xml 반환 함수 개수: {len(cleaned_functions)}")
            for i, func in enumerate(cleaned_functions):
                if i == 2:  # 세 번째 함수(CASE 문)만 확인
                    print(f"🔍 반환 함수 {i}: {func[:200]}...")
    ...
    def transform_function_for_testing_with_parser(self, func):
        ...
        try:
            ...
            if self.debug:
                print(f"🔍 파서 처리 오류: {e}, 기본 처리 사용")
                print(f"🔍 오류 발생 함수: {func[:100]}...")
            # 기본적인 처리: 컬럼명을 1로 치환
            result = func
            # 테이블.컬럼 패턴 치환
            result = re.sub(r'\b[A-Za-z][A-Za-z0-9_]*\.[A-Za-z0-9_]+\b', '1', result, flags=re.IGNORECASE)
            # 단독 컬럼명 치환 (예약어와 함수명 제외)
            words = re.findall(r'\b[A-Za-z_][A-Za-z0-9_]*\b', result)
            for word in set(words):
                if (word.upper() not in self.mysql_reserved_words and 
                    word.upper() not in self.all_functions and 
                    word != '1'):
                    result = re.sub(r'\b' + re.escape(word) + r'\b(?!\s*\()', '1', result, flags=re.IGNORECASE)
            return result
    ...
    def clean_sql_before_execution(self, sql):
        ...
        for i, op in enumerate(comparison_operators):
            ...
            for j, match in enumerate(reversed(matches)):
                ...
                if self.debug:
                    print(f"🔒 연산자 보호: '{match.group()}' → '{placeholder}'")
        
        if self.debug and operator_protections:
            print(f"🔒 총 {len(operator_protections)}개 연산자 보호됨")
        ...
        # 4단계: 연산자 복원
        if self.debug and operator_protections:
            print(f"🔓 연산자 복원 시작: {len(operator_protections)}개")
        for placeholder, original in operator_protections:
            if placeholder in sql:
                sql = sql.replace(placeholder, original)
                if self.debug:
                    print(f"🔓 연산자 복원: '{placeholder}' → '{original}'")
            elif self.debug:
                print(f"⚠️ 연산자 placeholder 찾을 수 없음: '{placeholder}'")
        ...
        # 1. 먼저 가장 복잡한 패턴부터 처리 (3개 이상의 1, 공백 허용)
        sql = re.sub(r'\bORDER\s+BY\s+1\s*,\s*1\s*,\s*1\s*(?:,\s*1\s*)*', 'ORDER BY (SELECT NULL)', sql, flags=re.IGNORECASE)
        sql = re.sub(r'\bPARTITION\s+BY\s+1\s*,\s*1\s*,\s*1\s*(?:,\s*1\s*)*', 'PARTITION BY (SELECT NULL)', sql, flags=re.IGNORECASE)
    ...
    def process_file(self, xml_file_path):
        ...
        try:
            ...
            if self.debug:
                ...
                for i, func in enumerate(original_functions):
                    print(f"  함수 {i}: {func[:100]}...")
                    if i == 2 and '>' in func:
                        print(f"    ✅ 함수 {i}에 > 연산자 있음")
                    elif i == 2:
                        print(f"    ❌ 함수 {i}에 > 연산자 없음")
            ...
            for i, func in enumerate(original_functions):
                if self.debug:
                    print(f"🔍 변환 시작 함수 {i}: {func[:100]}...")
                
                transformed_func = self.transform_function_for_testing_with_parser(func)
                transformed_functions.append(transformed_func)
                
                if self.debug:
                    print(f"🔍 변환 완료 함수 {i}: {transformed_func[:100] if transformed_func else 'None'}...")
