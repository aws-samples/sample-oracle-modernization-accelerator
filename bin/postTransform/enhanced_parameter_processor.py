...
# -*- coding: utf-8 -*-
"""
강화된 파라미터 처리기 - jdbcType, typeHandler 등 복잡한 표현식 지원
"""
...
class EnhancedParameterProcessor:
    def __init__(self):
    ...
    def extract_parameters_comprehensive(self, content: str) -> List[str]:
        """포괄적인 파라미터 추출"""
    ...
    def get_smart_default_value(self, param_name: str) -> Any:
        """스마트 기본값 결정 - jdbcType 고려"""
    ...
    def substitute_parameters_comprehensive(self, content: str) -> str:
        """포괄적인 파라미터 치환"""
        
        # 1단계: 파라미터 추출
        parameters = self.extract_parameters_comprehensive(content)
        
        # 2단계: 기본값 설정
        param_values = {}
        for param in parameters:
            param_values[param] = self.get_smart_default_value(param)
        ...
        # foreach 처리 먼저
        result = self._process_foreach_comprehensive(result, param_values)
        
        # 일반 파라미터 치환
        result = self._substitute_regular_parameters_comprehensive(result, param_values)
    ...
    def _process_foreach_comprehensive(self, content: str, param_values: Dict) -> str:
        """포괄적인 foreach 처리"""
    ...
    def _substitute_regular_parameters_comprehensive(self, content: str, param_values: Dict) -> str:
        """포괄적인 일반 파라미터 치환"""
    ...
    def _safe_parameter_replace(self, content: str, pattern: str, value: Any, 
        ...
        def replace_func(match):
            ...
            if isinstance(value, str):
                if is_literal:
                    return value  # ${} 는 리터럴 치환
                else:
                    return f"'{value}'"  # #{} 는 따옴표 추가
...
def test_enhanced_processor():
    ...
    processor = EnhancedParameterProcessor()
    
    test_cases = [
        # jdbcType 포함 파라미터
        """
        SELECT * FROM TB_TEST 
        WHERE PAY_TL = #{payTl,jdbcType=VARCHAR}
        AND SEQNO = #{pnrSeqno,jdbcType=INTEGER}
        """,
        
        # 복잡한 foreach
        """
        <foreach collection="listAreaRouteCd" item="item" open="(" separator="," close=")">
    ...
    for i, test_case in enumerate(test_cases):
        ...
        print("\n출력:")
        result = processor.substitute_parameters_comprehensive(test_case)
        print(result)
