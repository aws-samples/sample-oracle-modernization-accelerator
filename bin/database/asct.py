...
def setup_logging():
    ...
    if not DBMS_LOGS_FOLDER:
        print("ERROR: DBMS_LOGS_FOLDER environment variable not set")
        sys.exit(1)
    
    os.makedirs(DBMS_LOGS_FOLDER, exist_ok=True)
    log_file = os.path.join(DBMS_LOGS_FOLDER, 'asct.log')
...
# Initialize logger after environment validation
logger = None

# Environment variables
DBMS_FOLDER = os.environ.get('DBMS_FOLDER')
DBMS_LOGS_FOLDER = os.environ.get('DBMS_LOGS_FOLDER')
...
def validate_environment():
    """Validate required environment variables."""
    required_vars = {
        'DBMS_FOLDER': DBMS_FOLDER,
        'DBMS_LOGS_FOLDER': DBMS_LOGS_FOLDER,
        'ORACLE_ADM_USER': ORACLE_ADM_USER,
        'ORACLE_ADM_PASSWORD': ORACLE_ADM_PASSWORD,
        'ORACLE_SVC_USER': ORACLE_SVC_USER,
    ...
    os.makedirs(DBMS_FOLDER, exist_ok=True)
    os.makedirs(DBMS_LOGS_FOLDER, exist_ok=True)
    
    # Check for ORACLE_AURORA_POSTGRESQL*.zip file
    zip_files = glob.glob(os.path.join(DBMS_FOLDER, 'ORACLE_AURORA_POSTGRESQL*.zip'))
    if not zip_files:
        logger.error(f"Required ORACLE_AURORA_POSTGRESQL*.zip file not found in {DBMS_FOLDER}")
        logger.error("Please place the DMS Schema Conversion assessment file in the assessments folder")
        return False
    
    logger.info(f"Found assessment file: {zip_files[0]}")
    return True

# Set Oracle connection string after validation
ORACLE_CONNECTION = None
if ORACLE_ADM_USER and ORACLE_ADM_PASSWORD and ORACLE_HOST and ORACLE_PORT and ORACLE_SID:
...
def find_csv_files():
    ...
    zip_files = glob.glob(os.path.join(DBMS_FOLDER, 'ORACLE_AURORA_POSTGRESQL*.zip'))
    if not zip_files:
        logger.error(f"No ORACLE_AURORA_POSTGRESQL*.zip files found in {DBMS_FOLDER}")
        return None
    ...
    extract_dir = os.path.join(DBMS_FOLDER, 'extracted_csv')
...
def save_incompatible_list(objects_by_type):
    ...
    output_file = os.path.join(DBMS_FOLDER, 'incompatible.lst')
...
def convert_to_postgres(oracle_ddl, object_type, object_name, strict_mode=False):
    ...
    try:
        ...
        prompt_template_path = os.path.join(os.path.dirname(__file__), 'oracle_to_postgres_prompt.txt')
        ...
        debug_file = os.path.join(DBMS_LOGS_FOLDER, f'debug_{object_name.split(".")[-1]}.txt')
...
def save_original_ddl(object_type, object_name, oracle_ddl):
    ...
    oracle_dir = os.path.join(DBMS_FOLDER, 'oracle')
...
def save_failed_conversion(object_type, object_name, oracle_ddl, error_message=None):
    ...
    failed_dir = os.path.join(DBMS_FOLDER, 'failed_conversions')
...
def save_conversion_results(object_type, object_name, postgres_ddl):
    ...
    transform_dir = os.path.join(DBMS_FOLDER, 'transform')
    ...
    if result.returncode != 0:
        ...
        if "syntax error" in result.stderr:
            ...
            if obj_type and obj_name:
                # 원본 Oracle DDL 파일 경로
                oracle_file = os.path.join(DBMS_FOLDER, 'oracle', f"{obj_name.split('.')[-1]}.sql")
...
def execute_sql_in_postgres(sql_file):
    ...
    try:
        ...
        if result.returncode != 0:
            ...
            error_log_file = os.path.join(DBMS_LOGS_FOLDER, 'postgres_errors.log')
...
def analyze_postgres_errors():
    ...
    # 보고서 생성
    report_file = os.path.join(DBMS_LOGS_FOLDER, 'postgres_error_report.txt')
...
def main():
    ...
    # Check Oracle connection variables
    if not ORACLE_CONNECTION:
        logger.error("Oracle connection environment variables not properly set")
        sys.exit(1)
    ...
    incompatible_file = os.path.join(DBMS_FOLDER, 'incompatible.lst')
