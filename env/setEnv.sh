...
read_properties() {
    ...
    while read -r line || [ -n "$line" ]; do
        ...
        if [ "$in_project_section" = true ] && [[ -n $key && -n $value ]]; then
            ...
            # Export as session environment variable
            export "$env_var"="$expanded_value"
...
if [ ! -f "./oma.properties" ]; then
    echo -e "${RED}Error: oma.properties file not found in current directory.${NC}"
    exit 1
fi
...
# Create environment variable file in current directory only
...
if [ ! -f "./oma.properties" ]; then
    echo -e "${YELLOW}Warning: oma.properties file not found in current directory. Skipping dynamic variable extraction.${NC}"
else
...
echo -e "${YELLOW}Environment variable file location:${NC}"
echo -e "${GREEN}  - Current directory: $ENV_FILE${NC}"
echo -e "${CYAN}You can source this file to load environment variables.${NC}"
