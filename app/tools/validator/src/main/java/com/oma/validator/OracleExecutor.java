 ...
 * Executes MyBatis mappers against Oracle database with auto-rollback
 */
public class OracleExecutor implements DatabaseExecutor {
    ...
    public OracleExecutor(String mapperDir) throws Exception {
    ...
    private SqlSessionFactory createSqlSessionFactory() throws Exception {
        ...
        String host = getEnvOrDefault("ORACLE_HOST", "localhost");
        String port = getEnvOrDefault("ORACLE_PORT", "1521");
        String serviceName = getEnvOrDefault("ORACLE_SID", "ORCL");
        String connType = getEnvOrDefault("ORACLE_CONN_TYPE", "service");
        String username = getEnvOrDefault("ORACLE_USER", "system");
        String password = getEnvOrDefault("ORACLE_PASSWORD", "oracle");

        // Build JDBC URL based on connection type
        String jdbcUrl;
        if ("sid".equalsIgnoreCase(connType)) {
        ...
        String configXml = createMyBatisConfig(jdbcUrl, username, password, "oracle.jdbc.OracleDriver", actualMapperDir);
        File tempConfig = File.createTempFile("mybatis-config-oracle-", ".xml");
    ...
    public String execute(String sqlId, Map<String, Object> params) throws Exception {
        ...
        try (SqlSession session = sqlSessionFactory.openSession(false)) {
            ...
            try {
                ...
                session.rollback();
                throw new Exception("Oracle execution failed for " + sqlId + ": " + e.getMessage(), e);
            }
        }
    }
    ...
    private String processMapperFilesWithExtension() throws IOException {
        ...
        File tempDir = Files.createTempDirectory("mappers-oracle-").toFile();
