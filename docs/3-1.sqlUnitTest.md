...
| 용도 | 환경 변수 | 실제 경로 | 설명 |
|------|-----------|-----------|------|
| 테스트 작업 디렉토리 | TEST_FOLDER | /home/ec2-user/workspace/sample-oracle-modernization-accelerator/ourhome-itsm/test | 모든 테스트 파일이 저장되는 기본 디렉토리 |
| 테스트 로그 디렉토리 | TEST_LOGS_FOLDER | /home/ec2-user/workspace/sample-oracle-modernization-accelerator/ourhome-itsm/logs/test | 테스트 실행 로그 저장 |
| XML 리스트 디렉토리 | APP_TRANSFORM_FOLDER | /home/ec2-user/workspace/sample-oracle-modernization-accelerator/ourhome-itsm/application/transform | XML 파일 목록 검색 |
...
```bash
export ORACLE_SVC_USER=ITSM_APP
export ORACLE_SVC_PASSWORD=ITSM_APP191
export ORACLE_HOST=ip-10-50-2-27.ap-northeast-2.compute.internal
export ORACLE_PORT=1522
export ORACLE_SVC_CONNECT_STRING=orcl
export ORACLE_SID=OHOASSP
```

#### PostgreSQL 연결 정보

```bash
export PGHOST=omabox-stack-aurora-cluster.cluster-cv28wq0iksko.ap-northeast-2.rds.amazonaws.com
export PGPORT=5432
export PGDATABASE=ohpitsm
export PGUSER=itsm_app
export PGPASSWORD=ITSM_APP191
```
