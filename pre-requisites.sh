...
setup_oma_environment() {
    ...
    if [ ! -f "./01.Pre-requisites/deploy-omabox.sh" ]; then
        echo -e "${RED}오류: ./01.Pre-requisites/deploy-omabox.sh 파일을 찾을 수 없습니다.${NC}"
        return 1
    fi
    
    echo -e "${BLUE}${BOLD}실행 명령:${NC}"
    echo -e "${BLUE}${BOLD}./01.Pre-requisites/deploy-omabox.sh${NC}"
    print_separator

    echo -e "${BLUE}${BOLD}작업을 수행하기 이전에 3초 대기 합니다.${NC}"
    sleep 3

    ./01.Pre-requisites/deploy-omabox.sh
    print_separator
