"""
Direct Oracle→MySQL Data Transfer Tools
...
# ── Connection helpers (reuse logic from oracle_tools / mysql_tools) ──
...
async def _get_pg_conn():
    """Create MySQL connection using env vars or Secrets Manager."""
    from common.tools.mysql_tools import get_pg_connection
...
def migrate_table_data(table_name: str, schema: str = None, batch_size: int = 5000,
    ...
    """
    Migrate data from an Oracle table directly to MySQL without returning row data.

    Uses MySQL COPY protocol (via aiomysql copy_records_to_table) for bulk insert,
    ...
    Args:
        ...
        target_pg_schema: Target MySQL schema (default: derived from OMA_PG_USER or oracle schema)
    ...
    async def _migrate():
        ...
        try:
            ...
            # 2. Connect to MySQL and get column metadata
            ...
            # 3. Stream from Oracle → COPY to MySQL in chunks
            rows_exported = 0
            rows_inserted = 0
            errors = 0
            first_error = None
            ...
            while True:
                ...
                # Bulk COPY using aiomysql's COPY protocol
                try:
...
def migrate_all_tables(schema: str = None, truncate_first: bool = True,
    ...
    """
    Migrate ALL tables from Oracle schema to MySQL in one call.
    ...
    async def _migrate_all():
        try:
            ...
            try:
                from common.tools.mysql_tools import pg_sync_sequences
...
def verify_data_integrity(schema: str = None) -> dict:
    """Cross-check Oracle vs MySQL row counts for ALL tables.
            ...
            return {
                ...
                "cross_reference": {
                    ...
                    "note": (
                        ...
                        "information_schema equivalents in MySQL."
                    ),
                },
            }
