"""Schema Architect Agent -- Designs MySQL schema from Oracle DDL.

The Schema Architect Agent receives the Discovery Report and produces
MySQL-compatible DDL with proper data type mappings, constraints,
...
SYSTEM_PROMPT = """\
You are the **Schema Architect Agent** for Oracle-to-MySQL migration.

Your mission is to design MySQL DDL ONLY for objects that DMS Schema
Conversion could not handle. DMS SC has already applied DDL to MySQL
...
This tool queries both Oracle and MySQL databases and returns the
GROUND TRUTH of what exists vs what's missing. It returns:
- `pg_exists`: Objects already in MySQL → DO NOT TOUCH
- `agent_required`: Objects missing from MySQL → YOU MUST CONVERT THESE
...
2. Converted Oracle DDL to MySQL DDL (rule-based)
3. Applied converted DDL directly to MySQL (`export_to_target`)
...
- `dms_sc_apply_ddl` -- Apply DMS-converted DDL scripts to MySQL (fallback)
...
- `pg_execute_ddl` -- Execute DDL on MySQL (for validation)
...
- `converted_ddls`: List of MySQL DDL statements in dependency order
...
@dataclass
class TypeMapping:
    """A single Oracle -> MySQL type mapping decision."""
...
def build_schema_task(discovery_report_json: str) -> str:
    ...
    """
    return (
        "Design a MySQL schema based on the following Oracle Discovery Report.\n"
        "\n"
        "Requirements:\n"
        "1. Convert ALL objects from the inventory to MySQL DDL.\n"
        "2. Map every Oracle data type to the most appropriate MySQL type.\n"
