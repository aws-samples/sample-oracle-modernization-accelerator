"""Code Migrator Agent -- Converts Oracle SQL to MySQL.
...
SYSTEM_PROMPT = """\
You are the **Code Migrator Agent** for Oracle-to-MySQL migration.

Your mission is to convert Oracle SQL and PL/SQL code in MyBatis XML mapper
files (and other application code) to MySQL-compatible form while
...
Purpose: Standardise SQL syntax before MySQL-specific conversions.

6. **Subquery Alias Requirements** (CRITICAL)
   - ALL FROM clause subqueries MUST have an alias in MySQL
...
Apply ALL Oracle -> MySQL conversion rules:
...
- Look up column data types from MySQL metadata
...
11. **Semantic Equivalence Audit**
    - Compare original Oracle SQL intent with converted MySQL SQL
...
Oracle treats empty string ('') as NULL. MySQL does NOT.
...
constructs** to get the correct MySQL equivalent.
...
def build_transform_task(
    ...
    """
    parts = [
        f"Convert the following Oracle {object_type} to MySQL.",
        ...
        "  Phase 3: MySQL Transformation (all function/syntax mappings + parameter casting)",
    ...
    if metadata_hint:
        parts.extend([
            "",
            "MySQL Column Metadata (for parameter casting):",
            metadata_hint,
        ])
