...
"""
AP03.SQLTransformTarget.py - MyBatis XML 변환 대상 처리 프로그램
...
2. 각 XML 파일을 추출, 변환, 병합하는 과정 수행
    - 추출 : AP03.xmlExtractor.py
    - 변환 : q chat --trust-all-tools --no-interactive < {temp_prompt_file} > {log_file}
    - 병합 : AP03.xmlMerger.py
3. 변환 결과 검증
    - xmllint 유효성 검사
    

사용법:
    python3 AP03.SQLTransformTarget.py [옵션]
...
예제:
    python3 AP03.SQLTransformTarget.py -f /path/to/SQLTransformTarget.csv
    python3 AP03.SQLTransformTarget.py --file /path/to/SQLTransformTarget.csv --origin-suffix _orcl --transform-suffix _pg
"""
...
def process_xml_file(xml_file, origin_suffix, transform_suffix, mapper_folder, origin_mapper_folder, prompt_file, qlog_folder, qprompt_folder, log_level, logger):
    ...
    try:
        ...
        # 4. AP03.xmlExtractor.py 호출을 위한 출력 폴더 구성
        ...
        # 5. AP03.xmlExtractor.py 호출 - 로그 레벨 전달
        extractor_cmd = [
            "python3",
            os.path.join(os.path.dirname(os.path.abspath(__file__)), "AP03.xmlExtractor.py"),
        ...
        if run_command(extractor_cmd, logger) != 0:
            logger.error(f"AP03.xmlExtractor.py failed for {cp_target_path}")
            return False, None
        ...
        # 11. AP03.xmlMerger.py 호출 - 로그 레벨 전달
        merger_cmd = [
            "python3",
            os.path.join(os.path.dirname(os.path.abspath(__file__)), "AP03.xmlMerger.py"),
        ...
        if run_command(merger_cmd, logger) != 0:
            logger.error(f"AP03.xmlMerger.py failed for {xmltransform_folder}")
            return False, None
...
def main():
    ...
    # 로그 파일 경로 설정
    if not args.log:
        log_file = os.path.join(pylog_folder, f"AP03_sqlTransformTarget_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log")
    else:
        log_file = args.log
    ...
    validation_script = os.path.join(os.path.dirname(os.path.abspath(__file__)), "AP03.TransformValidation.py")
    ...
    try:
        ...
        validation_log_file = os.path.join(pylog_folder, f"AP03_TransformValidation_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log")
