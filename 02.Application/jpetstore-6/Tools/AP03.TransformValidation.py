...
"""
AP03.TransformValidation.py - MyBatis XML 변환 검증 프로세스
...
사용법:
    python3 AP03.TransformValidation.py [옵션]
...
예제:
    python3 AP03.TransformValidation.py -m /path/to/mapper -f /path/to/transform
    python3 AP03.TransformValidation.py --mapper-folder /path/to/mapper --origin-suffix _orcl --transform-folder /path/to/transform
"""
...
def main():
    ...
    if test_mode:
        # 테스트 모드의 기본값
        transform_folder = '/home/ec2-user/environment/OMA/Application/jpetstore-6/Transform'
        mapper_folder = '/home/ec2-user/environment/OMA/Application/jpetstore-6/Transform/mapper'
        log_level = logging.DEBUG
        log_level_str = 'DEBUG'
    else:
    ...
    # 로그 파일 설정
    if not args.log:
        log_file = os.path.join(log_folder, f"AP03_TransformValidation_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log")
    else:
        log_file = args.log
