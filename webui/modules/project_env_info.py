...
def render_project_env_page():
    """Project environment info page"""
    st.markdown("## 📊 Project Environment Info")
    show_project_environment_info()


def show_project_environment_info():
    """Display project environment info in table format"""
    # Load current config file
    config, _ = st.session_state.oma_controller.load_saved_config()
    env_vars = config.get('env_vars', {})
    
    if not env_vars:
        st.warning("⚠️ No saved environment info found. Please select a project first.")
        return
    ...
    if table_data:
        ...
        # Summary info
        st.info(f"📊 Total **{len(env_vars)}** environment variables are configured.")
    else:
